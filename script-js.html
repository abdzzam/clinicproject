...
// close sale dan print kertas
// paypad button melayang
    ...
    addOrderLine(quantity, sku) {
        ...
        for (let i = 0; i < this._menu.length; i++) {
            if (sku === this._menu[i].sku) {
                ...
                currentLine.tax = this._menu[i].taxRate * currentLine.subtotal;
            }
        }
...
class Ui {
    static menu(orderInstance) {
        ...
        orderInstance.menu.forEach(item => {
            ...
            <th class="stok-item">${item.image}</th>
...
class Utilities {
    ...
    static convertFloatToString(float) {
        ...
        let formattedPrice = float.toLocaleString("id-ID", priceParams);
        let result = formattedPrice.replace(",00", "");
